...
def fill_UE_array(array, event, lepton_index):
    ...
    for i in range(6):
        ...
        for particle in event:
            ...
            if (
                (deltaR(particle.p(), lepton_vec) < 0.50)
                and (particle.isFinal())
                and (particle.pT() > 0.1)
                and (
                    (particle.isNeutral() and (particle.pAbs() > 0.8))
                    or
                    (particle.isCharged() and (particle.pAbs() > 1.5))
                )
            ):
                # Find max pT object
...
ofile_name = "ttbarProduction_UE_10MeVMin.root"
target_pid = -24
...
for iEvent in range(nEvent):
    ...
    for index, particle in enumerate(pythia.event):
        if particle.statusAbs() == 23: # Final State Particles
            if pythia.event[index].id() in lepton_pid_array:
                ilepton_minus = index
            elif pythia.event[index].id() in -1*lepton_pid_array:
                ilepton_plus = index
