...
# Files
ifile_name = "/data/home/ganowak/MG5_aMC_v2_9_25/ttbar_NLO_Test1/Events/run_10_decayed_1/events_PYTHIA8_0.hepmc" 
...
pdfrwgt_str = 'nnpdf31lo/F:ct18lo/F:msht20lo/F:nnpdf31nlo/F:ct18nlo/F:msht20nlo/F:pdf4lhc21/F'
...
pdfrwgt_array = np.array([0]*7, dtype=np.float32)
...
with pyhepmc.open(ifile_name) as f:
    for event in f:
        ...
        for particle in event.particles:
            if particle.pid==antitop_pid:
                antitop = particle
            elif particle.pid==-1*antitop_pid:
                top = particle
        ...
        for daughter in top.children:
            if daughter.pid in -1*jet_array:
                ...
                wplus = daughter
        for daughter in antitop.children:
            if daughter.pid in jet_array:
                antijet = daughter
            else:
                wminus = daughter
